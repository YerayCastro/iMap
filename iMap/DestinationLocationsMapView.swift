...
struct DestinationLocationsMapView: View {
    ...
    var body: some View {
        ...
        VStack {
            LabeledContent {
                TextField("Enter the destination", text: $destination.name)
        ...
        .safeAreaInset(edge: .bottom) {
            VStack {
                Toggle(isOn: $isManualMarker) {
                    Label("Tap marker placement is: \(isManualMarker ? "ON" : "OF")", systemImage: isManualMarker ? "mappin.circle" : "mappin.slash.circle")
                }
                .bold()
                .toggleStyle(.button)
